...
export function updateLesson(courseId: number, lessonId: number, data: LessonRequest) {
  return apiClient.put<ApiResponse<Lesson>>(`/${COURSE_BASE_V1}/${courseId}/lessons/${lessonId}`, data);
}

export function deleteLesson(courseId: number, lessonId: number) {
  return apiClient.delete<ApiResponse<void>>(`/${COURSE_BASE_V1}/${courseId}/lessons/${lessonId}`);
}
