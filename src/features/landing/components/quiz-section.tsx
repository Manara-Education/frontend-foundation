...
export function QuizSection() {
  ...
  return (
    <section id="quiz" dir="rtl" style={{ paddingBlock: "clamp(72px, 10vw, 100px)", paddingInline: "clamp(16px, 4vw, 28px)", background: BG }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(300px, 100%), 1fr))", gap: 60, alignItems: "center" }}>
          <FadeIn delay={0.1}>
            <div>
              ...
              <button className="min-h-11 lg:min-h-0" onClick={() => setShowHint(!showHint)} style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 7, fontFamily: FONT, fontSize: 13, color: PRIMARY, background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 32, display: "flex", flexDirection: "column", gap: 16 }}>
                {["أسئلة متعددة الخيارات مرتبطة بالمحتوى", "تغذية راجعة فورية عند الإجابة", "تلميح ذكي عند التوقف — دون كشف الإجابة", "تتبع نتائج الاختبارات عبر الدورة"].map((item) => (
                  <div key={item} style={{ display: "flex", alignItems: "flex-start", gap: 12, minInlineSize: 0 }}>
                    <CheckCircle2 size={16} color="#22C55E" style={{ flexShrink: 0, marginTop: 2 }} />
                    <span className="rs-longform" style={{ fontFamily: FONT, fontSize: 14, color: TEXT_MUTED, lineHeight: 1.6 }}>{item}</span>
                  </div>
                ))}
              </div>
            </div>
