...
export function InstructorMockup() {
  return (
    <div className="rs-longform" style={{ background: "#fff", borderRadius: 24, border: `1px solid ${BORDER}`, boxShadow: "0 8px 40px rgba(78,91,146,0.1)", overflow: "hidden", fontFamily: FONT, direction: "rtl" }}>
      <div style={{ background: `linear-gradient(135deg, ${PRIMARY} 0%, ${PRIMARY_DARK} 100%)`, paddingBlock: 18, paddingInline: "clamp(18px, 6vw, 24px)" }}>
      ...
      <div style={{ paddingBlock: 20, paddingInline: "clamp(18px, 6vw, 24px)" }}>
        {[
          ...
          <div key={step} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, minInlineSize: 0 }}>
            <div style={{ inlineSize: 32, blockSize: 32, borderRadius: 99, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 13, background: done ? `${PRIMARY}18` : active ? PRIMARY : "rgba(78,91,146,0.06)", color: done ? PRIMARY : active ? "#fff" : TEXT_LIGHT, border: active ? "none" : done ? `1.5px solid ${PRIMARY}22` : `1.5px solid rgba(78,91,146,0.12)` }}>
        ...
        <div style={{ marginTop: 8, padding: "12px 16px", borderRadius: 12, background: BG_SOFT, border: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
            {[{ n: "٨", l: "دروس" }, { n: "٣", l: "اختبارات" }, { n: "٤", l: "أقسام" }].map(({ n, l }) => (
              <div key={l} style={{ textAlign: "center", padding: "6px 0", borderRadius: 8, background: "#fff", border: `1px solid ${BORDER}`, minInlineSize: 0 }}>
