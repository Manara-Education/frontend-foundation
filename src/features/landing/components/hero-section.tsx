...
export function HeroSection({ onCta }: { onCta: () => void }) {
  return (
    <section id="hero" dir="rtl" style={{ paddingTop: 120, paddingBottom: 80, background: `linear-gradient(180deg, ${BG_SOFT} 0%, ${BG} 100%)` }}>
      <div style={{ maxInlineSize: 1200, margin: "0 auto", paddingInline: "clamp(16px, 4vw, 28px)", display: "grid", gap: 60, alignItems: "center", gridTemplateColumns: "repeat(auto-fit, minmax(min(300px, 100%), 1fr))" }}>
        <FadeIn>
          <div className="rs-longform">
            ...
            <p style={{ fontFamily: FONT, fontSize: 17, color: TEXT_MUTED, lineHeight: 1.85, margin: "0 0 36px", maxInlineSize: 480 }}>
            ...
            <div id="hero-cta" style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
              <button onClick={onCta} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 52, maxInlineSize: "100%", paddingInline: 28, borderRadius: 14, background: `linear-gradient(135deg, ${PRIMARY} 0%, #6172AC 100%)`, color: "#fff", fontFamily: FONT, fontWeight: 700, fontSize: 15, border: "none", cursor: "pointer", boxShadow: "0 4px 20px rgba(78,91,146,0.3)" }}>
                انضم للوصول المبكر <ArrowLeft size={16} />
              </button>
              <button onClick={() => document.querySelector("#how-it-works")?.scrollIntoView({ behavior: "smooth" })} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 52, maxInlineSize: "100%", paddingInline: 24, borderRadius: 14, background: "transparent", color: PRIMARY, border: `1.5px solid ${BORDER}`, fontFamily: FONT, fontWeight: 600, fontSize: 15, cursor: "pointer" }}>
